...
export default function SeatingCanvas({
    ...
    const calculateCurvedPositions = () => {
        ...
        return riserSections.map((section, index) => {
            ...
            // Tiny gap to prevent visual overlap (0.5 degrees)
            const gapAngleRad = (0.5 * Math.PI) / 180;
            const gapAngleDeg = 0.5;
    ...
    return (
        <div className="relative w-full h-full bg-gray-900 flex items-center justify-center overflow-hidden">
            ...
            <div className="absolute bottom-6 right-6 flex flex-col gap-2 bg-gray-800/80 backdrop-blur rounded-lg p-2 border border-white/10 shadow-xl z-50">
                <button onClick={() => {
                    const canvas = document.querySelector('.seating-canvas-content');
                    if (canvas) {
                        const currentScale = parseFloat(canvas.style.transform?.match(/scale\(([^)]+)\)/)?.[1] || '1');
                        canvas.style.transform = `scale(${Math.min(currentScale * 1.2, 3)})`;
                    }
                }} className="p-2 hover:bg-white/10 rounded transition-colors" title="Zoom In">
                ...
                <button onClick={() => {
                    ...
                    if (canvas) {
                        const currentScale = parseFloat(canvas.style.transform?.match(/scale\(([^)]+)\)/)?.[1] || '1');
                        canvas.style.transform = `scale(${Math.max(currentScale / 1.2, 0.5)})`;
                    }
                }} className="p-2 hover:bg-white/10 rounded transition-colors" title="Zoom Out">
                ...
                }} className="p-2 hover:bg-white/10 rounded transition-colors" title="Reset View">
                    <RotateCcw className="w-5 h-5 text-white" />
                </button>
            </div>

            {/* Zero-size container at the exact center of the viewport */}
            <div className="relative w-0 h-0 seating-canvas-content transition-transform duration-200">
                    ...
                    <>
                        {isCurved ? (
                            ...
                            <div
                                ...
                                style={{
                                    ...getPositionStyle(0, directorY + 300), // Position 300px above director in math coordinates
